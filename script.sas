...
PROC SQL;
	CREATE TABLE WORK.JOINED_PREP_DATA AS
		SELECT
			t1.TransactionDate,
			t1.Unit,
			t1.UnitActual,
			...
			(CASE   WHEN t1.CustomerSatisfaction ge 0 and t1.CustomerSatisfaction lt 0.33 THEN 'A'   WHEN t1.CustomerSatisfaction ge 0.33 and t1.CustomerSatisfaction lt 0.66 THEN 'B'   WHEN t1.CustomerSatisfaction ge 0.66 and t1.CustomerSatisfaction lt 1 THEN 'C' END) AS satisfaction_group,
			t1.FacilityRegion
		FROM
			WORK.JOINED_DATA t1
	;
...
PROC HPSPLIT DATA = JOINED_PREP_DATA ;
    CLASS satisfaction_group _CHARACTER_;
    MODEL satisfaction_group = _NUMERIC_ _CHARACTER_;
